"""
This module defines the functions related to handling favourite crags
...
def get_favourites(user_id):
    """Returns all crags favourited by the user"""
    sql = """
...
def is_in_favourites(user_id, crag_id):
    """Check if crag is already in favourite_crags of the user"""
    sql = """
    SELECT EXISTS (
    SELECT 1
...
def add_crag_to_favourites(user_id, crag_id):
    """Adds a crag to the user's favourite_crags (or changes deleted to FALSE)"""
    try:
        sql = """
...
def delete_from_favourites(user_id, crag_id):
    """Delete favourite crag by marking it deleted"""
    try:
        sql = """
        UPDATE favourite_crags
        SET deleted = TRUE
        WHERE user_id = :user_id
        AND crag_id = :crag_id"""
        db.session.execute(text(sql), {"user_id":user_id, "crag_id":crag_id})
...
def times_favourited(crag_id):
    """Returns count, so how many times the crag has been favourited by users"""
    sql = "SELECT COUNT(*) FROM favourite_crags WHERE crag_id = :crag_id AND deleted IS NOT TRUE"
